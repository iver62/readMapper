...
public class kBand {
	...
	private String genome, read;
	private int match, sub, gap, k;
	private int[][] sim;
	private char[][][] trace;
	private int n, m;
		...
		for (int i = 1; i <= n; i++) { // on remplit ligne par ligne
			for (int j = 1; j <= m; j++) {
				if (Math.abs(i-j) <= k) {
					...
					if (d == max) { // si le max est d
						trace[i][j][0] = 'd'; // on vient de la diagonale
					}
					if (t == max) { // si le max est t
	...
	public String align() {
		String resU = new String(); // u avec les eventuels points
		String resV = new String(); // v avec les eventuels points
